...
class FrdContent extends React.Component{
  ...
  acceptInv(event){
    ...
    console.log(JSON.stringify({inviterId:inviterId,userId:this.props.userId, IsAccepted:IsAccepted}))
    fetch(`${url}/friend/handleInvitation`,{
        method: "POST",
        headers: {
        'Content-Type': 'application/json'
        },
        body: JSON.stringify({inviterId:inviterId,userId:this.props.userId, IsAccepted:IsAccepted}),
        mode: 'cors'
    })
    .then(response =>{
  ...
  async deleteFrd(event){
    ...
    if(confirmation){
      await fetch(`${url}/friend/delete`,{
        ...
        body: JSON.stringify({userId:this.props.userId,friendId:event.target.value}),
        mode: 'cors'
      })
      .then(response =>{
          response.json().then(df=>{
        ...
        <div >
          <div id="frd list">
            <div className="d-flex p-2">
              ...
              <button className={this.state.showfrd?"btn btn-secondary btn-sm bi bi-chevron-up m-2":"btn btn-secondary btn-sm bi bi-chevron-down m-2"} onClick={this.handleFrdList}></button>
            </div>
              ...
              <table className = "table border text-light text-center">
                ...
                {this.state.frd.map((frd,index)=>
                  <tr key ={index}>
                    <td>{frd.userId}</td>
                    <td>{frd.username}</td>
                    <td><Link to={`/user/${this.props.userId}/searchUser/profile/${frd.userId}`}>Go to profile</Link></td>
                    <td><button type="button" onClick={this.deleteFrd} value = {frd.userId} className="btn-close btn-close-white" aria-label="Close"></button></td>
                  </tr>
                )}
                </tbody>
              </table>  
          ...
          <div id = "frd invitation" >
            <div className="d-flex p-2">
                ...
                <button className={this.state.showInv?"btn btn-secondary btn-sm bi bi-chevron-up m-2":"btn btn-secondary btn-sm bi bi-chevron-down m-2"} onClick={this.handleInvList}></button>
            </div>
            <table className={this.state.showInv?"table text-light border":"d-none"}>
              <thead>
                <th>Inviter Id</th>
                <th>Inviter </th>
                <th>Invited time</th>
              ...
              <tbody>
                  {this.state.inv.map((inv,index)=>
                    <tr key = {index}>
                      ...
                      <td><Fromnow date ={inv.time}/></td>
                      <td><button value = {true} inviter={inv.inviter.userId} onClick = {this.acceptInv} className = "btn btn-primary btn-sm">Accept</button></td>
                      <td><button value = {false} inviter={inv.inviter.userId} onClick = {this.acceptInv} className = "btn btn-danger btn-sm">Reject</button></td>
                    </tr>
                  )}
              </tbody>
            </table>
      ...
      <div className="container">
        ...
        <div id = "user info" className={this.state.update?"d-none":""}>
          ...
          <div className="d-flex justify-content-end">
            <button className= "button"onClick={this.setUpdate}>Go for update</button>
          </div>
        ...
        <div id = "udpate info" className={this.state.update?"":"d-none"}>
          <div className="d-flex justify-content-end">
          <button className= "button"onClick={this.cancelUpdate}>back</button>
          </div>
